...
vectornet_load_path = "bigmodel/vector_net_199.pt"
big_model_load_path = "bigmodel/big_model_199.pt"
...
with torch.no_grad():
    for j, data in enumerate(dataloader):
        ...
        print(j,y.shape[0])
        cache.update(dict(zip(seq_id.cpu().numpy().tolist(), traj.cpu().numpy().tolist())))
        
    with open(os.path.join(directory, "text.json"), "w") as file:
