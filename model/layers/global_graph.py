...
class GlobalGraph2(nn.Module):
    ...
    def forward(self, x, y, vilid_len):
        for layer in self.layers:
            x = layer(x, y, vilid_len)
...
class SelfAttentionFCLayer(nn.Module):
    ...
    def forward(self, x, valid_lens):
        ...
        x = torch.add(torch.bmm(attention_weights, value), x)
        return x
...
class SelfAttentionWithKeyFCLayer(nn.Module):
    def __init__(
        ...
        self.k_lin = nn.Linear(in_channels, global_graph_width)
        self.v_lin = nn.Linear(in_channels, global_graph_width)
        self.layer_norm = nn.LayerNorm(global_graph_width)
        self.scale_factor_d = 1 + \
            int(np.sqrt(self.in_channels)) if need_scale else 1
    ...
    def forward(self, x1, x2, valid_lens):
        ...
        return torch.add(x1, self.layer_norm(x))
