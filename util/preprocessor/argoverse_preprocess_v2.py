...
class ArgoversePreprocessor(Preprocessor):
    ...
    def trans(self, data):
        ...
        for i, [traj, has_obs, pred, has_pred] in enumerate(zip(trajs, has_obss, preds, has_preds)):
            ...
            for j, obs in enumerate(has_obs):
                if obs:
                    cache[j].append(
                        traj[obs].tolist() + [i]
                    )
            
            for j, obs in enumerate(has_pred):
                cache[j+20].append(
                        pred[obs].tolist() + [i]
                    )
        return line, cache
    ...
    def plot_traj(self, obs, pred, traj_id=None):
        ...
        if obj_type == 'AGENT':
            plt.plot(obs[:, 0], obs[:, 1], color='b', alpha=1, linewidth=1, zorder=15)
            plt.plot(pred[:, 0], pred[:, 1], color='red', alpha=1, linewidth=1, zorder=15)
        else:
        ...
        if len(pred) == 0:
            ...
            if obj_type == 'AGENT':
                plt.text(obs[-1, 0], obs[-1, 1], "{}_s".format(traj_na))
