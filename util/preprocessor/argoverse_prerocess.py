...
class ArgoversePreprocessor(Preprocessor):
    ...
    def process(self, dataframe: pd.DataFrame,  seq_id, map_feat=True):
        ...
        if self.label_dict:
            data["label"] = int(self.label_dict[str(seq_id) + ".csv"])
        # print(data)
        return pd.DataFrame(
    ...
    def read_argo_data(df: pd.DataFrame):
        ...
        data["agt_traj"] = agt_traj
        data["agt_step"] = agt_step
        return data
    ...
    def get_obj_feats(self, data):
        ...
        if self.normalized:
            ...
            # pre = np.mean(data['trajs'][0][:self.obs_horizon], 0) - np.array(data['trajs'][0][0])
            pre = np.array(data['agt_traj'][-1]) - np.array(data['agt_traj'][0])
        ...
        # agt_traj_obs = data['agt_traj'][0: self.obs_horizon].copy().astype(np.float32)
        # agt_traj_fut = data['trajs'][0][self.obs_horizon:self.obs_horizon+self.pred_horizon].copy().astype(np.float32)
        agt_traj_obs = data['agt_traj'].copy().astype(np.float32)
        ...
        box_max = np.max(agt_traj_obs, axis=0)
        box_min = np.min(agt_traj_obs, axis=0)

        agt_traj_obs = (agt_traj_obs - box_min) / (box_max - box_min)
        for i, _ in enumerate(ctr_line_candts):
            ctr_line_candts[i] = (np.matmul(rot, (ctr_line_candts[i] - orig.reshape(-1, 2)).T).T - box_min) / (box_max - box_min)

        tar_candts = self.lane_candidate_sampling(ctr_line_candts, [0, 0], viz=False)

        splines, ref_idx = self.get_ref_centerline(ctr_line_candts, agt_traj_obs)
        ...
        for traj, step in zip(data['trajs'], data['steps']):
            ...
            if len(traj) <= 1:
                continue
            traj_nd = (np.matmul(rot, (traj - orig.reshape(-1, 2)).T).T - box_min) / (box_max - box_min)
    ...
    def get_lane_graph(self, data):
        ...
        for lane_id in lane_ids:
            ...
            if x.max() < x_min or x.min() > x_max or y.max() < y_min or y.min() > y_max:
                ...
                lane.polygon = (np.matmul(data['rot'], (polygon[:, :2] - data['orig'].reshape(-1, 2)).T).T - box_min) / (box_max - box_min)
                lanes[lane_id] = lane
